...
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=980):
        self.inplanes = 64
        ...
        # 256 after layer5, 1024 after layer3
    ...
    def forward(self, x):
        x = self.conv1(x)
        x = self.bn1(x)  #1,64,224,224
        x = self.relu(x)
        x = self.maxpool(x)#1,64,112,112
        ...
        # x = x.view(x.size(0), -1)
        # x = self.fc(x)
        x = self.conv_end(x) # 1,21,28,28
        
        x = self.bn_end(x)
        x = torch.sigmoid(x)  #
        # x = x.view(-1,7,7,30)
        x = x.permute(0, 2, 3, 1)  # (-1,28,28,21)
